...
exports.TypeParsingContext = void 0;
const countries_1 = require("../schemas/countries");
...
class TypeParsingContext {
    ...
    swallowAir() {
        while (this.moreFood() && (0, chars_1.isSpace)(this.taste())) {
            this.getNextEdible();
        }
    }
    tastesLike(ch) {
    ...
    parseNumber() {
        ...
        while (this.moreFood() && (0, chars_1.isDigit)(this.taste())) {
            temp += this.swallow();
        }
    ...
    swallowStringAttributeArgument(attributeName) {
        this.expect("(", `For attribute '@${attributeName}', expected '('`);
        this.swallowAir();
        const value = this.swallowString(`Expected a string (with single quotes) after '@${attributeName}('`);
        this.swallowAir();
        this.expect(")", `For attribute '@${attributeName}', expected ')'`);
        return value;
    }
    swallowBooleanAttributeArgument(attributeName) {
        this.expect("(", `For attribute '@${attributeName}', expected '('`);
        this.swallowAir();
        const value = this.swallowBoolean(`Expected a boolean after '@${attributeName}('`);
        this.swallowAir();
        this.expect(")", `For attribute '@${attributeName}', expected ')'`);
        return value;
    }
    swallowOptionalStringAttributeArgument(attributeName) {
        if (this.tastesLike("(")) {
            this.swallow();
            this.swallowAir();
            const value = this.swallowString(`Expected a string (with single quotes) after '@${attributeName}('`);
    ...
    swallowCountryAttributeArgument(attributeName) {
        this.expect("(", `For attribute '@${attributeName}', expected '('`);
        this.swallowAir();
        const value = this.swallowString(`Expected a string (with single quotes) after '@${attributeName}('`);
        const { data: countryCode, success } = countries_1.CountryCodeSchema.safeParse(value);
        if (!success) {
    ...
    swallowStringListAttributeArgument(attributeName) {
        this.expect("(", `For attribute '@${attributeName}', expected '('`);
        this.swallowAir();
        const value = this.swallowMultipleStrings(`Expected strings (with single quotes) after '@${attributeName}('`);
        this.swallowAir();
        this.expect(")", `For attribute '@${attributeName}', expected ')'`);
        return value;
    }
    swallowNumberAttributeArgument(attributeName) {
        this.expect("(", `For attribute '@${attributeName}', expected '('`);
        this.swallowAir();
        const value = this.swallowNumber(`Expected number after '@${attributeName}('`);
        this.swallowAir();
        this.expect(")", `For attribute '@${attributeName}', expected ')'`);
        return value;
    }
    swallowNumberListAttributeArgument(attributeName) {
        this.expect("(", `For attribute '@${attributeName}', expected '('`);
        this.swallowAir();
        const value = this.expectMultipleNumbers(`Expected numbers after '@${attributeName}('`);
        this.swallowAir();
        this.expect(")", `For attribute '@${attributeName}', expected ')'`);
        return value;
    }
}
exports.TypeParsingContext = TypeParsingContext;
