"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.upcSchema = exports.calculateCheckDigit = void 0;
const zod_1 = require("zod");
// Function to calculate check digit for UPC
const calculateCheckDigit = (upc) => {
    const digits = upc.split("").map(Number);
    const sumOdd = digits
        .filter((_, index) => index % 2 === 0)
        .reduce((acc, curr) => acc + curr, 0);
    const sumEven = digits
        .filter((_, index) => index % 2 !== 0)
        .reduce((acc, curr) => acc + curr, 0);
    const total = sumOdd * 3 + sumEven;
    const mod = total % 10;
    return mod === 0 ? 0 : 10 - mod;
};
exports.calculateCheckDigit = calculateCheckDigit;
// Zod schema for validating UPC
exports.upcSchema = zod_1.z
    .string()
    .length(12)
    .regex(/^\d{12}$/, "UPC must be 12 digits")
    .refine((upc) => {
    const checkDigit = (0, exports.calculateCheckDigit)(upc.slice(0, 11)); // Only consider the first 11 digits
    return checkDigit === Number(upc[11]); // Compare check digit
}, {
    message: "Invalid UPC check digit",
});
